...
async def robloxlink():
    ...
    urlbutton = Button(label="Click To Verify", url=auth_url, style=discord.ButtonStyle.green)
    donebutton = Button(label="Done", style=discord.ButtonStyle.green)
    currentguild = interaction.guild
    async def donebuttoncallback (interaction):
        params = {
            'discord_id': str(interaction.user.id),
        }
