...
async def save_feedback():
    try:
        async with app.app_context():
            ...
            if admin == False:
                return jsonify({"error": "You are not an admin in this server"})
            if welcome_channel and welcome_role and welcome_message is not None:
                await ipc_client.request("save_feedback", guild_id=guild_id, welcome_channel=welcome_channel, welcome_role=welcome_role ,welcome_message=welcome_message , toggle_status=toggle_status)
