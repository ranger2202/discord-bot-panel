...
@app.route("/welcome/<int:guild_id>")
async def server_dashboard_welcome(guild_id):
    try:
        user = await discord.fetch_user()
        get_user_admin_in_guilds = await ipc_client.request("get_shared_guilds", user=user.id)
...
@app.route("/leaves/<int:guild_id>")
async def server_dashboard_leaves(guild_id):
    try:
        user = await discord.fetch_user()
        get_user_admin_in_guilds = await ipc_client.request("get_shared_guilds", user=user.id)
        ...
        return await render_template("leaves.html",
                                      username=user.name,
                                      user=session["discord_user"],
                                      guild_info=selected_guild_info,
                                      selected_guild_icon=selected_guild_icon,
    ...
    try:
        async with app.app_context():
            ...
            if feedback_channel is not None:
                await ipc_client.request("save_feedback", guild_id=guild_id, feedback_channel=feedback_channel, toggle_status=toggle_status)

                return jsonify({"message": "success"})
            else:
                return jsonify({"error": "No channel provided in the request"})
    except Exception as e:
        return jsonify({"error": f"An error occurred: {e}"})
...
async def save_welcome():
    try:
        async with app.app_context():
            ...
            welcome_role = data.get("welcome_role")  
            ...
            if welcome_channel  is not None:
                await ipc_client.request("save_welcome", guild_id=guild_id, welcome_channel=welcome_channel, welcome_role=welcome_role,welcome_message=welcome_message, toggle_status=toggle_status)
...
async def save_leave():
    try:
        async with app.app_context():
            data = await request.json
            leave_channel = data.get("leave_channel")  
            leave_role = data.get("leave_role")  
            toggle_status = data.get("toggle_status")  
            guild_id = data.get("guild_id")  
            try:
                user = await discord.fetch_user()
            ...
            if leave_channel  is not None:
                await ipc_client.request("save_welcome", guild_id=guild_id, leave_channel=leave_channel, welcome_role=welcome_role,welcome_message=welcome_message, toggle_status=toggle_status)
